...
1. ログインが完了するまで十分待ってください（ポップアップが閉じてメインページに戻るまで）
2. alphaXivのホームページにリダイレクトされたことを確認してください
3. ブラウザが完全に閉じる前に、手動でページを開いたままにしてみてください
4. もう一度 `pnpm run get-token` を実行してください

**ブラウザが自動で閉じてしまう場合:**

- Googleログインのポップアップが表示されたら、ログインを完了させてください
- ログイン後、alphaXivのメインページに戻るまで待ってください
- スクリプトが自動的にトークンを検出します
